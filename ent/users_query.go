...
func (uq *UsersQuery) QueryLogs() *DefautlDBQuery {
	query := (&DefautlDBClient{config: uq.config}).Query()
...
func (uq *UsersQuery) WithLogs(opts ...func(*DefautlDBQuery)) *UsersQuery {
	query := (&DefautlDBClient{config: uq.config}).Query()
...
func (uq *UsersQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Users, error) {
	...
	if query := uq.withLogs; query != nil {
		if err := uq.loadLogs(ctx, query, nodes,
			func(n *Users) { n.Edges.Logs = []*DefautlDB{} },
			func(n *Users, e *DefautlDB) { n.Edges.Logs = append(n.Edges.Logs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UsersQuery) loadLogs(ctx context.Context, query *DefautlDBQuery, nodes []*Users, init func(*Users), assign func(*Users, *DefautlDB)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(users.LogsTable)
		s.Join(joinT).On(s.C(defautldb.FieldID), joinT.C(users.LogsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(users.LogsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(users.LogsPrimaryKey[0]))
	...
	neighbors, err := withInterceptors[[]*DefautlDB](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
